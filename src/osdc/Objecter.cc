...
void Objecter::_send_linger(LingerOp *info)
{
  ...
  info->watch_lock.get_read(); // juslllllllllllll to read registered status
...
void Objecter::tick()
{
  ...
  do {
	...
	assert(op->session);
	if (op->stamp < cutoff) {
	  ldout(cct, 2) << " tid " << p->first << " on osd."
			<< op->session->osd << " is laggy" << dendl;
