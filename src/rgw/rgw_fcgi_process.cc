...
void RGWFCGXProcess::run()
{
	...
	if (socket_path.empty()) {
	    dout(0) << "ERROR: no socket server point defined, cannot start"
		    << " fcgi frontend" << dendl;
	    return;
	}
    }
	...
	/* this is necessary, as FCGX_OpenSocket might not return an error, but
	 * rather ungracefully exit */
	...
	if (chmod(path, 0777) < 0) {
	    dout(0) << "WARNING: couldn't set permissions on unix domain"
	      " socket" << dendl;
	}
	...
	if (sock_fd < 0) {
	    dout(0) << "ERROR: FCGX_OpenSocket (" << bind.c_str()
		    << ") returned " << sock_fd << dendl;
	    return;
	}
    }
