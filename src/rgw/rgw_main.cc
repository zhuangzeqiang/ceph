...
static void check_curl()
{
  ...
  derr << "WARNING: cross zone/region transfer performance may be affected"
       << dendl;
}
#endif
...
int usage()
{
  ...
  cerr << "  --debug-rgw=<log-level>/<memory-level>"
       << "  set radosgw debug level\n";
...
int main(int argc, const char **argv)
{
  ...
  if (apis_map.count("admin") > 0) {
    ...
    admin_resource->register_resource("replica_log",
				      new RGWRESTMgr_ReplicaLog);
